...
const TexSupport = ({text}) =>{
    ...
    text.slice(from).replaceAll(/(\$.*\$)/g,(match, _, offset, string) =>{
        ...
        res.push(<MathComponent key={`math-${offset}`} display={false} tex={match.slice(1, -1)}/>);
...
const DocArea = (props)=>{
    ...
    const { data } = useSWR(`${HOST}/api/problem/${id}`, fetcher, { suspense: true });
    ...
    return( 
        <>
            ...
            <Stack gap={3}>
                <Box>
                    <Heading as={'h2'} fontSize={'xl'}>題目說明</Heading>
                    <TexSupport text={content.description}/>
                </Box>
                <Box>
                    <Heading as={'h2'} fontSize={'xl'}>輸入說明</Heading>
                    <TexSupport text={content.input_description}/>
                </Box>
                <Box>
                    <Heading as={'h2'} fontSize={'xl'}>輸出說明</Heading>
                    <TexSupport text={content.output_description}/>
                </Box>
                <Box>
                    <Heading as={'h2'} fontSize={'xl'}>Note</Heading>
